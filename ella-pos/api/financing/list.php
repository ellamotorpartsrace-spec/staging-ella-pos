...
try {
    ...
    // Base query for financing sales (includes legacy home_credit AND new financing)
    $sql = "
        SELECT 
            s.sale_id,
            s.sale_ref,
            ...
            -- Fetch Financed Amount (from pos_sale_payments where type = financing or home_credit)
            (
                SELECT p.amount 
                FROM pos_sale_payments p 
                WHERE p.sale_id = s.sale_id AND p.payment_type IN ('home_credit', 'financing') 
                LIMIT 1
            ) as financed_amount,
            ...
            -- Fetch Financing Provider
            (
                SELECT COALESCE(p.financing_provider, 'Home Credit')
                FROM pos_sale_payments p 
                WHERE p.sale_id = s.sale_id AND p.payment_type IN ('home_credit', 'financing') 
                LIMIT 1
            ) as financing_provider,

            -- Fetch Down Payment Amount
            (
                SELECT p.amount
                FROM pos_sale_payments p 
        ...
        WHERE s.payment_method IN ('home_credit', 'financing')
    ...
    if (!empty($provider)) {
        $sql .= " AND EXISTS (
            ...
            WHERE p.sale_id = s.sale_id 
              AND p.payment_type IN ('home_credit', 'financing')
              AND COALESCE(p.financing_provider, 'Home Credit') = :provider
        )";
