<?php
/**
 * api/lazada/callback.php — OAuth callback from Lazada
 * Lazada redirects here after shop owner authorizes.
 * URL: /ella-pos/api/lazada/callback.php?code=xxx&shop_id=xxx
 */
...
try {
    ...
    if (empty($code) || empty($shopId)) {
        die("Authorization failed: Missing code or shop_id from Lazada.");
    }
    ...
    if (!$config) {
        die("No Lazada configuration found.");
    }
    ...
    $tokenResult = $lazada->getAccessToken($code, $shopId);
    ...
    // Log success
    $stmt = $conn->prepare("
        INSERT INTO lazada_sync_logs (event_type, source, status, created_at)
        VALUES ('token_refresh', ?, 'success', NOW())
    ");
    $stmt->execute(['OAuth authorization — Shop ID: ' . $shopId]);

    // Redirect to settings page with success
    header("Location: " . BASE_URL . "views/lazada/laz_settings.php?auth=success&shop_id=" . $shopId);
    exit;
