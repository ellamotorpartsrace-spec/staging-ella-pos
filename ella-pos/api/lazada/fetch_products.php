<?php
/**
 * api/lazada/fetch_products.php — Fetch ALL products from Lazada & save to DB
 * 
 * This pulls every product + variation from the authorized Lazada shop,
 * applies the correct SKU matching logic, and saves to lazada_product_mappings.
 * 
 * Matching Rule:
...
try {
    ...
    if (!$config || empty($config['access_token'])) {
        echo json_encode(['success' => false, 'error' => 'Lazada not authorized. Please connect your shop first.']);
        exit;
    }
    ...
    if (strtotime($config['token_expires_at']) - time() < 600) {
        ...
        if (isset($refreshResult['access_token'])) {
            ...
            throw new Exception("Lazada token expired and auto-refresh failed: " . $errorMsg . ". Please go to Settings and re-authorize.");
        }
    }
    ...
    if (in_array($mode, ['quick', 'price', 'stock'])) {
        ...
        // Lazada API allows up to 15 days. 7 days guarantees we catch any recent edits while staying fast.
    ...
    $listResult = $lazada->get('/api/v2/product/get_item_list', $apiParams, $accessToken, $shopId);

    if (isset($listResult['error']) && $listResult['error'] !== '' && $listResult['error'] !== 0) {
        echo json_encode(['success' => false, 'error' => 'Lazada API error: ' . ($listResult['message'] ?? json_encode($listResult['error']))]);
        exit;
    }
    ...
    if (!empty($items)) {
        ...
        // ═══════════════════════════════════════
        // STEP 2: Get base info for these items (chunked in batches of 50 as permitted by Lazada)
        // ═══════════════════════════════════════
        $itemInfoList = [];
        ...
        if (!empty($modelQueryParams)) {
            ...
            foreach ($modelQueryChunks as $mChunk) {
                $multiResults = $lazada->getMulti('/api/v2/product/get_model_list', $mChunk, $accessToken, $shopId);
        ...
        foreach ($itemInfoList as $item) {
            ...
            if ($hasModels) {
                ...
                foreach ($models as $model) {
                    ...
                    $allProducts[] = [
                        'lazada_item_id'        => $itemId,
                        'lazada_model_id'       => $model['model_id'] ?? null,
                        'lazada_product_name'   => $itemName,
                        'lazada_variation_name' => $varName ?: null,
                        'lazada_parent_sku'     => $itemSku,
                        'lazada_variation_sku'  => $varSku,
                        'has_variation'         => 1,
                        'lazada_stock'          => $stock,
                        'lazada_price'          => $price,
                        'lazada_image_url'      => $imageUrl,
                    ];
                }
            } else {
                // ═══════════════════════════════════════
                ...
                $allProducts[] = [
                    'lazada_item_id'        => $itemId,
                    'lazada_model_id'       => null,
                    'lazada_product_name'   => $itemName,
                    'lazada_variation_name' => null,
                    'lazada_parent_sku'     => $itemSku,
                    'lazada_variation_sku'  => null,
                    'has_variation'         => 0,
                    'lazada_stock'          => $stock,
                    'lazada_price'          => $price,
                    'lazada_image_url'      => $imageUrl,
    ...
    if (!empty($itemIds)) {
        ...
        $stmt = $conn->prepare("
            SELECT id, lazada_item_id, lazada_model_id, matched_pos_sku, pos_product_id, mapping_status, sync_hash 
            FROM lazada_product_mappings 
            WHERE lazada_item_id IN ($placeholders)
        ");
        $stmt->execute($itemIds);
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
            $key = $row['lazada_item_id'] . '_' . ($row['lazada_model_id'] ?? 'parent');
    ...
    foreach ($allProducts as $product) {
        ...
        $contentHash = md5(implode('|', [
            $product['lazada_product_name'],
            $product['lazada_variation_name'] ?? '',
            $product['lazada_parent_sku'] ?? '',
            $product['lazada_variation_sku'] ?? '',
            $product['lazada_stock'],
            $product['lazada_price'],
        ]));

        // Check if already exists in pre-fetched cache
        $modelId = $product['lazada_model_id'];
        $cacheKey = $product['lazada_item_id'] . '_' . ($modelId ?? 'parent');
        ...
        if ($existing) {
            if (isset($existing['sync_hash']) && $existing['sync_hash'] === $contentHash && $mode !== 'stock' && $mode !== 'price') {
                $conn->prepare("UPDATE lazada_product_mappings SET last_synced_at=NOW() WHERE id=?")
                    ->execute([$existing['id']]);
                $skipped++;
                continue;
            }
            ...
            if ($mode === 'stock') {
                $conn->prepare("UPDATE lazada_product_mappings SET lazada_stock=?, last_stock_sync_at=NOW(), last_synced_at=NOW() WHERE id=?")->execute([$product['lazada_stock'], $existing['id']]);
                ...
                if (($existing['mapping_status'] === 'auto' || $existing['mapping_status'] === 'manual') && !empty($posProductId)) {
                    $prodName = $product['lazada_product_name'] . (!empty($product['lazada_variation_name']) ? ' — ' . $product['lazada_variation_name'] : '');
                    $lazadaSku = $product['lazada_variation_sku'] ?: $product['lazada_parent_sku'] ?: '—';
                    propagateStockToPos($conn, (int)$posProductId, (int)$product['lazada_stock'], $prodName, $lazadaSku, $_SESSION['user_id'] ?? null, (int)$existing['id']);
                }
            ...
            if ($mode === 'price') {
                // Only update the cached lazada_price column — never touch POS price fields
                $conn->prepare("UPDATE lazada_product_mappings SET lazada_price=?, last_price_sync_at=NOW(), last_synced_at=NOW() WHERE id=?")->execute([$product['lazada_price'], $existing['id']]);
            ...
            if ($existing['mapping_status'] === 'manual') {
                ...
                $stmt = $conn->prepare("
                    UPDATE lazada_product_mappings SET
                        lazada_product_name=?, lazada_variation_name=?,
                        lazada_parent_sku=?, lazada_variation_sku=?,
                        has_variation=?, lazada_stock=?, lazada_price=?,
                        lazada_image_url=?, sync_hash=?, last_synced_at=NOW(), updated_at=NOW()
                    WHERE id=?
                ");
                $stmt->execute([
                    $product['lazada_product_name'], $product['lazada_variation_name'],
                    $product['lazada_parent_sku'], $product['lazada_variation_sku'],
                    $product['has_variation'], $product['lazada_stock'], $product['lazada_price'],
                    $product['lazada_image_url'], $contentHash, $existing['id']
                ]);
            } else {
                ...
                if ($existing['mapping_status'] !== 'unmapped' && $existing['mapping_status'] !== 'missing_sku' && $mappingStatus !== 'missing_sku') {
                    ...
                    // Let's just update the lazada-specific fields and leave POS ones alone if it was already mapped.
                }

                if ($existing['mapping_status'] === 'auto' || $existing['mapping_status'] === 'manual') {
                    // Just update Lazada fields, don't touch POS matching fields to avoid unlinking
                    $stmt = $conn->prepare("
                        UPDATE lazada_product_mappings SET
                            lazada_product_name=?, lazada_variation_name=?,
                            lazada_parent_sku=?, lazada_variation_sku=?,
                    ...
                    $stmt = $conn->prepare("
                        UPDATE lazada_product_mappings SET
                            ...
                            lazada_parent_sku=?, lazada_variation_sku=?,
                            has_variation=?, lazada_stock=?, lazada_price=?,
                            lazada_image_url=?, matched_pos_sku=?, pos_product_id=?,
                    ...
                    $stmt->execute([
                        ...
                        $product['lazada_parent_sku'], $product['lazada_variation_sku'],
                        $product['has_variation'], $product['lazada_stock'], $product['lazada_price'],
                        $product['lazada_image_url'], $matchedPosSku, $posProductId,
            ...
            $stmt = $conn->prepare("
                INSERT INTO lazada_product_mappings (
                    lazada_item_id, lazada_model_id, lazada_product_name, lazada_variation_name,
                    lazada_parent_sku, lazada_variation_sku, has_variation,
                    lazada_stock, lazada_price, lazada_image_url,
                    matched_pos_sku, pos_product_id, mapping_status, stock_allocation_ratio, sync_hash, last_synced_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 100, ?, NOW())
            ");
            $stmt->execute([
                $product['lazada_item_id'], $product['lazada_model_id'],
                $product['lazada_product_name'], $product['lazada_variation_name'],
                $product['lazada_parent_sku'], $product['lazada_variation_sku'],
                $product['has_variation'], $product['lazada_stock'], $product['lazada_price'],
                $product['lazada_image_url'], $matchedPosSku, $posProductId, $mappingStatus, $contentHash
            ]);
            $inserted++;
        }
    }
    ...
    if ($queueId) {
        ...
        // Direct, unqueued sync - skip logging to avoid cluttering with Lazada-side imports
    }

    $responseJson = json_encode([
        'success'      => true,
        'message'      => 'Successfully synced products from Lazada',
