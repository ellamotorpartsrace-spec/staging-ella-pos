<?php
// api/lazada/allow_duplicate.php
...
try {
    ...
    // 1. Add to whitelist
    $stmt = $conn->prepare("INSERT IGNORE INTO lazada_duplicate_whitelist (sku) VALUES (?)");
    $stmt->execute([$sku]);

    // 2. Mark error as resolved
    $stmt2 = $conn->prepare("UPDATE lazada_error_logs SET status = 'resolved', resolved_at = NOW() WHERE error_type = 'duplicate_sku' AND sku = ? AND status = 'open'");
    $stmt2->execute([$sku]);
    ...
    if ($posVar) {
        $updateStmt = $conn->prepare("
            ...
            WHERE mapping_status NOT IN ('manual', 'auto')
              AND (
                  (has_variation = 0 AND lazada_parent_sku = ?) OR 
                  (has_variation = 1 AND lazada_variation_sku = ?)
              )
        ");
        ...
        // with mapId = null so it sums up all their lazada_stock and allocates it to the POS inventory.
    ...
    // 4. Record action in lazada sync logs
    $namesStmt = $conn->prepare("
        SELECT lazada_product_name, lazada_variation_name 
    ...
    foreach ($sharedItems as $si) {
        $n = $si['lazada_product_name'];
        if (!empty($si['lazada_variation_name'])) {
            $n .= ' — ' . $si['lazada_variation_name'];
        }
        $allowedNames[] = $n;
    }
    ...
    $logStmt = $conn->prepare("
        INSERT INTO lazada_sync_logs (event_type, product_name, sku, source, status, new_value, created_by, created_at)
