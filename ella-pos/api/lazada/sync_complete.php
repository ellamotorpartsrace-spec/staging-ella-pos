...
try {
    ...
    $stmt = $conn->prepare("UPDATE lazada_sync_queues SET status = ?, completed_at = NOW() WHERE id = ?");
    $stmt->execute([$status, $queueId]);

    // Automatically execute conflict detection to update lazada_error_logs and stats in real-time
    if ($status === 'completed') {
        ...
        $qStmt = $conn->prepare("SELECT * FROM lazada_sync_queues WHERE id = ?");
        ...
        if ($queue) {
            ...
            $conn->prepare("
                INSERT INTO lazada_sync_logs (event_type, product_name, source, status, new_value, created_by, created_at)
                VALUES ('product_import', ?, 'Lazada API Import', 'success', ?, ?, NOW())
            ")->execute([
                "Lazada Catalog Synchronization ({$modeLabel})",
