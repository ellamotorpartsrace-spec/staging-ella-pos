<?php
/**
 * api/lazada/save_config.php — Save Lazada API credentials
 */
...
try {
    ...
    if ($existing) {
        ...
        $stmt = $conn->prepare("
            INSERT INTO lazada_config (environment, partner_id, partner_key, shop_region, low_stock_threshold, out_of_stock_alerts, buffer_stock, is_active)
            VALUES (?, ?, ?, ?, ?, ?, ?, 1)
        ");
        $stmt->execute([$environment, $partnerId, $partnerKey, $shopRegion, $lowStockThresh, $oosAlerts, $bufferStock]);
    }

    echo json_encode(['success' => true, 'message' => 'Lazada credentials saved (' . strtoupper($environment) . ' mode)']);
