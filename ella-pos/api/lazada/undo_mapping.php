...
try {
    ...
    if ($action === 'link') {
        ...
        $updateStmt = $conn->prepare("UPDATE lazada_product_mappings SET pos_product_id = NULL, matched_pos_sku = NULL, mapping_status = 'unmapped' WHERE id = ?");
        foreach ($mappingIds as $id) {
            ...
            if ($item) {
                ...
                ");
                $lazadaSku = $item['lazada_variation_sku'] ?: $item['lazada_parent_sku'] ?: '—';
                $logStmt->execute([
                    $item['lazada_item_id'],
                    $prodName,
                    $lazadaSku,
        ...
        $fetchStmt = $conn->prepare("SELECT lazada_item_id, lazada_product_name, lazada_variation_name, lazada_variation_sku, lazada_parent_sku FROM lazada_product_mappings WHERE id = ?");
        $auditStmt = $conn->prepare("SELECT old_value FROM lazada_audit_logs WHERE action_type = 'bulk_unlink' AND target_id = ? ORDER BY created_at DESC LIMIT 1");
        $skuStmt = $conn->prepare("SELECT sku FROM product_variations WHERE variation_id = ?");
        $updateStmt = $conn->prepare("UPDATE lazada_product_mappings SET pos_product_id = ?, matched_pos_sku = ?, mapping_status = 'manual' WHERE id = ?");
