...
try {
    ...
    $mappingsStmt = $conn->query("
        SELECT m.lazada_item_id, m.lazada_model_id, m.pos_product_id, m.pos_bundle_set_id, m.pos_unit_id, u.price_capital, u.multiplier 
    ...
    foreach ($orderSnChunksForStatus as $snChunk) {
        ...
        $stmt = $conn->prepare("SELECT order_sn, order_status, inventory_deducted FROM lazada_orders WHERE order_sn IN ($placeholders)");
    ...
    foreach ($orderDetails as $order) {
        ...
        $isCancelled = in_array($orderStatus, ['CANCELED', 'CANCELLED', 'RETURNED']);
        ...
        $stmtOrder->execute([
            $orderSn,
            $order['order_status'], // keep original case for DB
            $createTime,
            $updateTime,
        ...
        if (!empty($variationsToUpdate)) {
            ...
            $remarks = $shouldDeduct ? 'Lazada Order' : 'Lazada Order Cancelled/Returned';
