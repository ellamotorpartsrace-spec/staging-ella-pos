<?php
/**
 * api/lazada/cleanup_orphan_online_stock.php
 *
 * One-time cleanup: finds POS products that have online stock (store_id=2)
 * but NO active Lazada mapping (status='auto' or 'manual'), then zeroes out
...
try {
    ...
    // but are NOT currently linked to any active Lazada mapping.
    $stmt = $conn->query("
    ...
    foreach ($orphans as $row) {
        ...
        $conn->prepare("
            ...
            VALUES (?, 2, 'allocation_to_physical', ?, ?, 0, ?, 'Cleanup: removed orphan online allocation (no active Lazada link)', ?, 0)
        ")->execute([
            $varId,
            -$onlineQty,
            $onlineQty,
