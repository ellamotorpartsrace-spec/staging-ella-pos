<?php
/**
 * api/lazada/save_mappings.php — Save Lazada product mappings to DB
 */
...
try {
    ...
    $oldStmt = $conn->prepare("SELECT lazada_item_id, lazada_model_id, lazada_product_name, lazada_variation_name, lazada_variation_sku, lazada_parent_sku, matched_pos_sku, pos_product_id, pos_unit_id, pos_bundle_set_id, mapping_status, lazada_stock FROM lazada_product_mappings WHERE id = ?");

    $autoMatchedCount = 0; // Counter for items auto-matched in this operation
    $stmt = $conn->prepare("UPDATE lazada_product_mappings SET 
        matched_pos_sku = ?, 
        pos_product_id = ?, 
        pos_unit_id = ?,
        pos_bundle_set_id = ?,
    ...
    foreach ($mappings as $map) {
        ...
        $newBundleSetId = normalizeLazadaBundleSetId($conn, $map['posBundleSetId'] ?? null);
        $newPosId = $newBundleSetId ? null : (!empty($map['posId']) ? (int)$map['posId'] : null);
        $newUnitId = normalizeLazadaPosUnitId($conn, $newPosId, $map['posUnitId'] ?? null);
        ...
        if ($hasChanged && $oldMap) {
            ...
            $skuVal = $oldMap['lazada_variation_sku'] ?: $oldMap['lazada_parent_sku'] ?: '—';
            ...
            // Propagate live Lazada stock and price back into local POS tables immediately
            if (($newStatus === 'manual' || $newStatus === 'auto') && !empty($newPosId)) {
                ...
                // Only fetch live stock from Lazada API if it's a single manual link.
                ...
                if ($shouldFetchLive) {
                    try {
                        // Instantly query Lazada API for the true live stock
                        ...
                        $updCache = $conn->prepare("UPDATE lazada_product_mappings SET lazada_stock = ?, last_synced_at = NOW(), updated_at = NOW() WHERE id = ?");
                        ...
                        $warnStmt = $conn->prepare("
                            ...
                            VALUES ('mapping', ?, ?, ?, 'failed', ?, 'Live Lazada Fetch Fallback', ?, NOW())
                        ");
                        $warnStmt->execute([
                            $oldMap['lazada_item_id'],
                            $prodName,
                            $skuVal,
                            'Failed to fetch live Lazada stock: ' . $liveEx->getMessage(),
                            $_SESSION['user_id'] ?? null
                        ]);
                    }
                }
                ...
                $checkSkuForWhitelist = $oldMap['lazada_variation_sku'] ?: $oldMap['lazada_parent_sku'] ?: null;
                ...
                if ($isSharedListing) {
                    // Shared listing: preserve the real Lazada stock value for display
