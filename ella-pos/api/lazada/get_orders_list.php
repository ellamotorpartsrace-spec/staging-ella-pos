<?php
/**
 * api/lazada/get_orders_list.php
 */
...
try {
    ...
    if (!empty($search)) {
        $where .= " AND (o.order_sn LIKE ? OR o.buyer_username LIKE ? OR EXISTS (
            SELECT 1 FROM lazada_order_items i WHERE i.order_sn = o.order_sn AND (i.item_sku LIKE ? OR i.item_name LIKE ?)
        ))";
        $searchTerm = "%$search%";
    ...
    $sql = "
        SELECT 
            ...
            (SELECT SUM(capital_cost * quantity_purchased) FROM lazada_order_items WHERE order_sn = o.order_sn) as total_capital,
            (SELECT GROUP_CONCAT(CONCAT(quantity_purchased, 'x ', item_name) SEPARATOR ', ') FROM lazada_order_items WHERE order_sn = o.order_sn) as items_summary
