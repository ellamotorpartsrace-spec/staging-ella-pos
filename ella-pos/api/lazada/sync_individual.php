<?php
/**
 * api/lazada/sync_individual.php — Sync a single Lazada item (pull info + push stock)
 */
...
try {
    ...
    // 1. Get current mapping and Lazada config with both physical (store_id = 1) and online (store_id = 2) quantities
    $stmt = $conn->prepare("SELECT m.*, c.partner_id, c.partner_key, c.shop_id, c.access_token, c.environment, c.buffer_stock, c.out_of_stock_alerts,
        ...
        JOIN lazada_config c ON c.is_active = 1
    ...
    $lazada = new LazadaAPI($item['partner_id'], $item['partner_key'], $isTest);
    $accessToken = $item['access_token'];
    $shopId = $item['shop_id'];

    // 2. Refresh info from Lazada
    $infoResult = $lazada->get('/api/v2/product/get_item_base_info', [
        'item_id_list' => $item['lazada_item_id']
    ], $accessToken, $shopId);

    $lazadaItem = $infoResult['response']['item_list'][0] ?? null;
    if ($lazadaItem) {
        // Update price and generic info
        $price = $lazadaItem['price_info'][0]['current_price'] ?? $item['lazada_price'];
        $conn->prepare("UPDATE lazada_product_mappings SET lazada_price = ?, updated_at = NOW() WHERE id = ?")
            ->execute([$price, $id]);
    }
    ...
    $newPhysicalStock = $totalQty - $computedStock;
    ...
    if (!empty($item['pos_product_id'])) {
        ...
        $movementStmt = $conn->prepare("
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, 'Lazada Stock Allocation Sync (Individual)', ?, ?)
        ");
    ...
    if ($pushedStock == 0 && (int)$item['lazada_stock'] > 0 && !empty($item['out_of_stock_alerts'])) {
        $prodName = $item['lazada_product_name'];
        if (!empty($item['lazada_variation_name'])) {
            $prodName .= ' — ' . $item['lazada_variation_name'];
        }
    ...
    if ($item['lazada_model_id']) {
        $stockItem['model_id'] = (int)$item['lazada_model_id'];
    }

    $syncResult = $lazada->post('/api/v2/product/update_stock', [
        'item_id' => (int)$item['lazada_item_id'],
    ...
    // 4. Log the success in lazada_sync_logs
    $conn->prepare("INSERT INTO lazada_sync_logs (event_type, lazada_item_id, product_name, sku, old_value, new_value, source, status, created_by, created_at) VALUES ('stock_update', ?, ?, ?, ?, ?, 'Manual Sync (Individual)', 'success', ?, NOW())")
        ->execute([
            $item['lazada_item_id'],
            $item['lazada_product_name'], 
    ...
    try {
        if (isset($conn)) {
            $logStmt = $conn->prepare("INSERT INTO lazada_sync_logs (event_type, lazada_item_id, product_name, sku, status, error_message, created_by, created_at) VALUES ('stock_update', ?, ?, ?, 'failed', ?, ?, NOW())");
            $logStmt->execute([
                isset($item) ? $item['lazada_item_id'] : null,
                isset($item) ? $item['lazada_product_name'] : null,
