<?php
/**
 * api/lazada/cleanup_deleted.php
 * Cross-references local database with Lazada API to detect and remove deleted products/variations.
 */
...
try {
    ...
    // 3. Get all distinct Lazada Item IDs in our database
    $stmt = $conn->prepare("SELECT DISTINCT lazada_item_id FROM lazada_product_mappings");
    $stmt->execute();
    ...
    foreach ($chunks as $chunk) {
        ...
        $baseInfoResult = $lazada->get('/api/v2/product/get_item_base_info', [
        ...
        foreach ($chunk as $itemId) {
            ...
            if (!isset($activeItems[$itemIdStr])) {
                // Item has been completely deleted from Lazada!
                // Fetch affected POS products to restore their stock
                $stmtGetAffected = $conn->prepare("SELECT DISTINCT pos_product_id FROM lazada_product_mappings WHERE lazada_item_id = ? AND pos_product_id IS NOT NULL AND mapping_status IN ('auto', 'manual')");
                $stmtGetAffected->execute([$itemIdStr]);
                $affectedPosIds = $stmtGetAffected->fetchAll(PDO::FETCH_COLUMN);

                $conn->prepare("DELETE FROM lazada_product_mappings WHERE lazada_item_id = ?")->execute([$itemIdStr]);
                ...
                if ($activeItems[$itemIdStr]['has_model']) {
                    ...
                    $stmtGetAffected = $conn->prepare("SELECT DISTINCT pos_product_id FROM lazada_product_mappings WHERE lazada_item_id = ? AND (lazada_model_id IS NULL OR lazada_model_id = 0) AND pos_product_id IS NOT NULL AND mapping_status IN ('auto', 'manual')");
                    $stmtGetAffected->execute([$itemIdStr]);
                    $affectedPosIds = $stmtGetAffected->fetchAll(PDO::FETCH_COLUMN);

                    $delStmt = $conn->prepare("DELETE FROM lazada_product_mappings WHERE lazada_item_id = ? AND (lazada_model_id IS NULL OR lazada_model_id = 0)");
        ...
        if (!empty($itemsWithModels)) {
            ...
            $modelResults = $lazada->getMulti('/api/v2/product/get_model_list', $modelQueries, $accessToken, $shopId);
            ...
            foreach ($itemsWithModels as $index => $iid) {
                ...
                $varStmt = $conn->prepare("SELECT lazada_model_id, pos_product_id, mapping_status FROM lazada_product_mappings WHERE lazada_item_id = ? AND lazada_model_id > 0");
                $varStmt->execute([$iid]);
                $localModels = $varStmt->fetchAll(PDO::FETCH_ASSOC);
                
                foreach ($localModels as $localModel) {
                    $localModelIdStr = (string)$localModel['lazada_model_id'];
                    if (!in_array($localModelIdStr, $activeModelIds)) {
                        // Variation was deleted on Lazada!
                        $conn->prepare("DELETE FROM lazada_product_mappings WHERE lazada_model_id = ?")->execute([$localModelIdStr]);
    ...
    $logStmt = $conn->prepare("INSERT INTO lazada_sync_logs (event_type, product_name, source, status, new_value, created_by, created_at) VALUES ('product_import', 'Ghost Product Cleanup', 'Manual Cleanup', 'success', ?, ?, NOW())");
