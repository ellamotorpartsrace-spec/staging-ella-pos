<?php
/**
 * api/lazada/update_allocation.php — Update stock allocation and push to Lazada
 */
...
try {
    ...
    // 2. Update the mapping's lazada_stock FIRST so the SUM query below includes the new value
    ...
    // 3. Compute POS online stock = SUM of all (lazada_stock * multiplier) sharing the same SKU (or same POS ID if SKU is empty)
    ...
    // 4. Prepare Lazada API call — send the individual listing's stock (not the total) minus buffer
    ...
    $response = $lazada->post('/api/v2/product/update_stock', $body, $item['access_token'], $item['shop_id']);
    ...
    $logStmt = $conn->prepare("INSERT INTO lazada_sync_logs (event_type, lazada_item_id, product_name, sku, old_value, new_value, source, status, created_by, created_at) 
    ...
    $conn->commit();
    echo json_encode(['success' => true, 'message' => 'Ratio updated and stock synced to Lazada']);
