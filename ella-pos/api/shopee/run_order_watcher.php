...
try {
    ...
    $scriptPath = __DIR__ . '/../../scripts/shopee_order_watcher.php';
    ...
    $logStmt = $conn->prepare("INSERT INTO shopee_sync_logs (event_type, source, status, created_by, created_at) VALUES ('order_sync', 'Manual Sync (Orders)', 'success', ?, NOW())");
    $logStmt->execute([$_SESSION['user_id'] ?? null]);

    echo json_encode([
        'success' => true,
        'message' => 'Shopee orders successfully synced!',
        'log' => $output
    ]);
} catch (Exception $e) {
    ...
    try {
        $db = new Database();
        $conn = $db->getConnection();
        $logStmt = $conn->prepare("INSERT INTO shopee_sync_logs (event_type, source, status, error_message, created_by, created_at) VALUES ('order_sync', 'Manual Sync (Orders)', 'failed', ?, ?, NOW())");
