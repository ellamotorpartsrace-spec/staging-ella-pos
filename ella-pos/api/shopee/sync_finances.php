<?php
/**
 * api/shopee/sync_finances.php — Fetch financial (escrow) details for completed orders
 */
...
try {
    ...
    $stmtFin = $conn->prepare("
        INSERT INTO shopee_financial_transactions (
            ...
            seller_voucher, shopee_voucher, escrow_release_time
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    foreach ($orderSns as $order_sn) {
        $escrowResult = $shopee->get('/api/v2/payment/get_escrow_detail', [
            'order_sn' => $order_sn
        ], $accessToken, $shopId);
        ...
        if (isset($escrowResult['response']['order_income'])) {
            ...
            $shopee_voucher = $income['shopee_voucher_code'] ?? 0.0;
