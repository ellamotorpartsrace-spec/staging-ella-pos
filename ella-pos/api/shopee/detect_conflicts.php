...
function runConflictDetection($conn) {
    ...
    $markActiveOrInsert = function($type, $itemId, $modelId, $sku, $message) use (&$remainingOpen, $conn, &$errorsCount) {
        ...
        if ($foundId !== null) {
            ...
            $conn->prepare("UPDATE shopee_error_logs SET error_message = ?, sku = ? WHERE id = ?")->execute([$message, $sku, $foundId]);
        } else {
            // New error, insert it
            $conn->prepare("
    ...
    // 2.5 Detect Duplicate SKUs within Shopee (two shopee items with the same SKU) using UNION ALL (optimized for indexes!)
    $stmt = $conn->query("
    ...
    foreach ($unmappedParents as $row) {
        if (isset($duplicateMap[$row['shopee_parent_sku']])) continue;
    ...
    foreach ($unmappedVars as $row) {
        if (isset($duplicateMap[$row['shopee_variation_sku']])) continue;
