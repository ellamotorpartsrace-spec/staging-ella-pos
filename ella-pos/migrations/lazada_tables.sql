-- ══════════════════════════════════════════════
-- LAZADA SYNC MODULE — DATABASE MIGRATION
-- Run this in phpMyAdmin on ella_parts_db
-- ══════════════════════════════════════════════

-- 1. Lazada API credentials and tokens
...
-- 2. Lazada product mappings
CREATE TABLE IF NOT EXISTS `lazada_product_mappings` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
    `lazada_item_id` BIGINT NOT NULL,
    `lazada_model_id` BIGINT DEFAULT NULL,
    `lazada_product_name` VARCHAR(500) DEFAULT NULL,
    `lazada_variation_name` VARCHAR(255) DEFAULT NULL,
    `lazada_parent_sku` VARCHAR(100) DEFAULT NULL,
    `lazada_variation_sku` VARCHAR(100) DEFAULT NULL,
    ...
    `lazada_stock` INT NOT NULL DEFAULT 0,
    `lazada_price` DECIMAL(10,2) DEFAULT NULL,
    `lazada_image_url` VARCHAR(1000) DEFAULT NULL,
    `last_synced_at` DATETIME DEFAULT NULL,
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX `idx_lazada_item` (`lazada_item_id`),
    INDEX `idx_lazada_model` (`lazada_model_id`),
