...
$mappedRows = $conn->query("
    SELECT m.id, m.shopee_item_id, m.shopee_product_name, m.shopee_variation_name,
        ...
        CAST((
            SELECT COALESCE(SUM(lm.lazada_stock * COALESCE(lu.multiplier, 1)), 0)
            FROM lazada_product_mappings lm
            LEFT JOIN product_units lu ON lm.pos_unit_id = lu.id
            WHERE (lm.pos_product_id = v.variation_id OR (v.sku NOT IN ('', '-', 'N/A', 'NA', 'none', 'null') AND lm.matched_pos_sku = v.sku COLLATE utf8mb4_unicode_ci))
              AND lm.mapping_status IN ('auto','manual','mapped')
              AND (lm.pos_bundle_set_id IS NULL OR lm.pos_bundle_set_id = 0)
...
if (!empty($bundleSetIds)) {
    ...
    foreach ($mappedRows as $mr) {
        ...
        $allocatedSets = (int)($mr['shopee_stock'] ?? 0);
...
.bg-shopee{background:var(--shopee-gradient) !important;}
.text-shopee{color:var(--shopee-primary) !important;}
.btn-shopee{background:var(--shopee-gradient) !important;border:none !important;color:#fff !important;transition:all .3s ease;}
.btn-shopee:hover{background:var(--shopee-gradient) !important;opacity:0.9;box-shadow:0 4px 12px rgba(238,77,45,0.35);color:#fff !important;}
.btn-outline-shopee-secondary{border-color:rgba(238,77,45,0.2) !important;color:var(--text-secondary) !important;background:transparent;}
.btn-outline-shopee-secondary:hover{border-color:var(--shopee-primary) !important;color:var(--shopee-primary) !important;}
.btn-light:hover{background-color:var(--border-color) !important;}



/* ── Enhanced Table Column Segments ── */
.sp-table th.col-pos { background: var(--bg-surface); }
.sp-table th.col-shopee { background: linear-gradient(180deg, rgba(238,77,45,.06) 0%, var(--bg-surface) 100%); }
.sp-table th.col-reserved { background: linear-gradient(180deg, rgba(245,158,11,.06) 0%, var(--bg-surface) 100%); }
.sp-table th.col-available { background: linear-gradient(180deg, rgba(16,185,129,.06) 0%, var(--bg-surface) 100%); }
.sp-table th.col-sep-left { border-left: 2px solid rgba(238,77,45,.15); }
.sp-table td.col-sep-left { border-left: 2px solid rgba(238,77,45,.08); }

/* ── Allocation Progress Bar ── */
.alloc-progress-wrap { width:100%; max-width:80px; margin:3px auto 0; }
.alloc-progress { height:4px; border-radius:2px; background:var(--sp-neutral-bg); overflow:hidden; }
...
.btn-live-sync:hover {
    border-color:var(--sp-info); color:var(--sp-info);
    background:var(--sp-info-bg); transform:rotate(30deg);
}
...
.sp-table .var-indent::after {
    content:''; position:absolute; left:1.8rem; top:50%; width:.8rem; height:1.5px;
    background: rgba(238,77,45,.25);
}
.var-connector-icon { color:var(--shopee-primary); font-size:.55rem; margin-right:.4rem; opacity:.6; }

/* ── Available to Sell Cell ── */
.avail-cell { font-weight:700; font-size:.9rem; }
.avail-cell.avail-good { color:var(--sp-success); }
.avail-cell.avail-low { color:var(--sp-warning); }
.avail-cell.avail-zero { color:var(--sp-danger); }
...
.status-chip.s-ok { background:var(--sp-success-bg); color:var(--sp-success); }
.status-chip.s-low { background:var(--sp-warning-bg); color:var(--sp-warning); }
.status-chip.s-out { background:var(--sp-danger-bg); color:var(--sp-danger); }
.status-chip.s-none { background:var(--sp-neutral-bg); color:var(--sp-neutral-text); }

/* ── Row entry animation ── */
@keyframes rowSlideIn { from { opacity:0; transform:translateX(-6px); } to { opacity:1; transform:translateX(0); } }
.sp-table tbody tr { animation: rowSlideIn .25s ease-out forwards; }
...
.reserved-chip.zero { background:var(--sp-neutral-bg); color:var(--sp-neutral-text); }
...
.sp-qty-control.border-shopee {
    border-color: rgba(238,77,45,0.25) !important;
    transition: all 0.2s ease;
}
.sp-qty-control.border-shopee:focus-within {
...
<div class="row g-3 mb-4">
    <div class="col-md-3 col-6">
        <div class="sp-stat-card accent-success">
            ...
            <div><div class="sp-stat-label">Allocated</div><div class="sp-stat-value" id="sumAllocated">0</div></div>
        </div>
    </div>
    <div class="col-md-3 col-6">
        <div class="sp-stat-card accent-warning">
            <div class="sp-stat-icon" style="background:var(--sp-warning-bg);color:var(--sp-warning)"><i class="fa-solid fa-triangle-exclamation"></i></div>
            <div><div class="sp-stat-label">Low Stock</div><div class="sp-stat-value" id="sumLowStock">0</div></div>
        </div>
    </div>
    <div class="col-md-3 col-6">
        <div class="sp-stat-card" style="border-left: 3px solid var(--sp-neutral-text);">
            <div class="sp-stat-icon" style="background:var(--sp-neutral-bg);color:var(--sp-neutral-text)"><i class="fa-solid fa-minus-circle"></i></div>
            <div><div class="sp-stat-label">Unallocated</div><div class="sp-stat-value" id="sumUnallocated">0</div></div>
        </div>
    </div>
    <div class="col-md-3 col-6">
        <div class="sp-stat-card accent-danger">
            <div class="sp-stat-icon" style="background:var(--sp-danger-bg);color:var(--sp-danger)"><i class="fa-solid fa-arrow-trend-up"></i></div>
            <div><div class="sp-stat-label">Overallocated</div><div class="sp-stat-value" id="sumOverallocated">0</div></div>
        </div>
    </div>
</div>
    ...
    <div class="sp-card mb-4" id="globalFilterCard">
        <div class="sp-card-body d-flex flex-wrap align-items-center gap-3" style="padding:.85rem 1.25rem">
            ...
            <div class="sp-filter-pills">
                <button class="sp-pill active" onclick="setAllocFilter('all',this)"><i class="fa-solid fa-list me-1"></i>All</button>
                <button class="sp-pill" onclick="setAllocFilter('manual',this)"><i class="fa-solid fa-hand-pointer me-1 text-info"></i>Manually Mapped</button>
                ...
                <button class="sp-pill" onclick="setAllocFilter('synced',this)"><i class="fa-solid fa-check-circle me-1 text-success"></i>Allocated</button>
                <button class="sp-pill" onclick="setAllocFilter('low',this)"><i class="fa-solid fa-triangle-exclamation me-1 text-warning"></i>Low Stock</button>
                <button class="sp-pill" onclick="setAllocFilter('unallocated',this)"><i class="fa-solid fa-minus-circle me-1 text-secondary"></i>Unallocated</button>
                <button class="sp-pill" onclick="setAllocFilter('overallocated',this)"><i class="fa-solid fa-arrow-trend-up me-1 text-danger"></i>Overallocated</button>
            </div>
        </div>
    </div>
    <div class="sp-card">
            ...
            <table class="sp-table">
                <thead>
                    <tr>
                        ...
                        <th style="width: 1%; white-space: nowrap;">Parent SKU</th>
                        <th style="width: 1%; white-space: nowrap;">Variation SKU</th>
                        <th class="text-center" style="width: 1%; white-space: nowrap;">Current Shopee Stock</th>
                        <th class="text-end" style="width: 1%; white-space: nowrap;">Action</th>
                    </tr>
                </thead>
            ...
            <div class="modal-body" style="padding: 1.25rem 1.5rem;">
                <div class="mb-3 pb-2.5 border-bottom" style="border-color: rgba(0,0,0,0.06) !important;">
                    ...
                    <div class="text-secondary small d-flex align-items-center gap-1.5" style="font-size: 0.72rem;">
                        <span>SKU:</span><span id="mSku" class="sp-sku-code">—</span>
                    </div>
                </div>
                    ...
                    <div class="col-12" id="modalMainCol">
                        <div class="d-flex align-items-stretch justify-content-between gap-2.5 mb-2">
                            ...
                            <div class="stock-summary-card flex-fill text-center p-3 rounded-3 border shadow-xs position-relative allocated-card" style="min-width: 0;">
                                <div class="stock-card-label text-uppercase text-shopee mb-1" style="font-size: 0.68rem; letter-spacing: 0.05em; font-weight: 700;"><i class="fa-brands fa-shopee me-1"></i>Allocated</div>
                                <div class="d-flex align-items-center justify-content-center my-1.5">
                                    <div class="sp-qty-control rounded bg-white border border-shopee d-flex align-items-center justify-content-between overflow-hidden" style="width: 110px; height: 32px;">
                                        <button class="sp-qty-btn text-shopee border-0 px-2.5 h-100" style="background: var(--shopee-light); font-size: 0.8rem;" onclick="adjStock(-1)"><i class="fa-solid fa-minus"></i></button>
                                        <input type="number" class="sp-qty-input fw-bold text-shopee border-0 text-center flex-fill h-100" style="font-size: 1.15rem; width: 40px; background: transparent; padding: 0;" id="mOnlineStock" value="0" min="0" oninput="calcModal()">
                                        <button class="sp-qty-btn text-shopee border-0 px-2.5 h-100" style="background: var(--shopee-light); font-size: 0.8rem;" onclick="adjStock(1)"><i class="fa-solid fa-plus"></i></button>
                                    </div>
                                </div>
                                <div class="stock-card-sub text-muted" style="font-size: 0.65rem; font-weight: 500;">On Shopee</div>
                            </div>
                        ...
                        <div class="mt-4 p-3 rounded-3 border bg-white shadow-xs" style="border-color: rgba(0,0,0,0.06) !important;">
                            <div class="d-flex justify-content-between align-items-center mb-2.5">
                                ...
                                <span id="mRatioText" class="badge bg-light text-shopee border fw-bold" style="font-size: 0.78rem; padding: 0.3rem 0.6rem;">0% on Shopee</span>
                            </div>
                            
                            <!-- Visual Progress Bar -->
                            <div class="progress rounded-pill overflow-hidden bg-light border mb-3.5" style="height: 12px;">
                                <div id="mBarShopee" class="progress-bar bg-shopee progress-bar-striped progress-bar-animated" role="progressbar" style="width: 0%;" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>
                            ...
                            <div class="d-flex flex-column gap-2.5" style="font-size: 0.85rem; line-height: 1.4;">
                                <!-- Shopee Statement -->
                                <div class="d-flex align-items-center gap-2">
                                    <div class="rounded-circle bg-shopee d-flex align-items-center justify-content-center text-white flex-shrink-0" style="width: 22px; height: 22px; font-size: 0.7rem;">
                                        <i class="fa-brands fa-shopee"></i>
                                    </div>
                                    <div class="text-dark">
                                        <strong id="mRatioLabelShopee" class="text-dark" style="font-size: 0.95rem;">0 items</strong> will be listed on your <strong>Shopee store</strong> for online buyers.
                                    </div>
                                </div>
...
function saveState() {
    ...
    sessionStorage.setItem('shopeeAllocState', JSON.stringify(state));
}
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    if (isReload) {
        const saved = sessionStorage.getItem('shopeeAllocState');
        if (saved) {
            try {
                ...
                if (state.activeTab === 'unmapped') {
                    const uBtn = document.querySelector('.sp-tab-btn[onclick*="unmapped"]');
                    if (uBtn) {
                ...
                const fBtn = document.querySelector(`.sp-pill[onclick*="'${allocFilter}'"]`) || document.querySelector(`.sp-pill[onclick*='"${allocFilter}"']`);
        ...
        sessionStorage.removeItem('shopeeAllocState');
    }
...
function renderModalSharedDetails(v) {
    ...
    itemsHtml += `
    <div class="d-flex align-items-center justify-content-between p-2 mb-2 rounded-2" style="background: linear-gradient(90deg, rgba(238,77,45,0.04) 0%, transparent 100%); border: 1px solid rgba(238,77,45,0.15); border-left: 3px solid var(--shopee-primary);">
    ...
    v.dupDetails.forEach(d => {
        ...
        const imgHtml = d.imageUrl 
            ? `<img src="${escHtml(d.imageUrl)}" class="sp-product-img me-2 shadow-sm" style="width: 32px; height: 32px; border-radius: 4px; border: 1px solid var(--border-color); object-fit: cover;">`
            : `<div class="sp-img-placeholder me-2 shadow-sm d-flex align-items-center justify-content-center bg-light" style="width: 32px; height: 32px; border-radius: 4px; border: 1px solid var(--border-color);"><i class="fa-solid fa-image text-secondary" style="font-size: 0.75rem;"></i></div>`;
        ...
        <div class="d-flex align-items-center justify-content-between p-2 mb-2 bg-white rounded-2 border shadow-xs" style="gap: 12px;">
            ...
            <div class="flex-shrink-0">
                <div class="sp-qty-control rounded bg-white border d-flex align-items-center justify-content-between overflow-hidden border-shopee" style="width: 105px; height: 32px;">
                    <button class="sp-qty-btn text-shopee border-0 px-2 h-100" style="background: var(--shopee-light); font-size: 0.8rem;" onclick="adjSharedStock(this, -1)"><i class="fa-solid fa-minus"></i></button>
                    <input type="number" class="sp-qty-input fw-bold text-center border-0 text-shopee flex-fill h-100 shared-alloc-input" style="font-size: 1rem; width: 35px; background: transparent; padding: 0;" data-id="${d.id}" data-orig="${d.online}" value="${d.online}" min="0" oninput="updateModalSharedDetailsAlert()">
                    <button class="sp-qty-btn text-shopee border-0 px-2 h-100" style="background: var(--shopee-light); font-size: 0.8rem;" onclick="adjSharedStock(this, 1)"><i class="fa-solid fa-plus"></i></button>
                </div>
            </div>
        </div>`;
            ...
            if (v) {
                ...
                html += `<tr class="sp-group-start">
                    ...
                    <td class="text-center">
                        ...
                        ${otherAlloc > 0 ? `<div class="mt-1" style="font-size: 0.65rem; color: #6c757d;">(-${otherAlloc.toLocaleString()} Lazada)</div>` : ''}
                    </td>
                    <td>${badge}</td>
                    <td class="text-end">
            ...
            vars.forEach(v=>{
                ...
                html += `<tr>
                    ...
                    <td class="text-center">
                        ...
                        ${other > 0 ? `<div class="text-muted font-normal lh-1 mt-1" style="font-size:0.65rem;" title="${other.toLocaleString()} reserved for Lazada">(-${other.toLocaleString()} Lazada)</div>` : ''}
                    </td>
                    <td>${badge}</td>
                    <td class="text-end">
                        <div class="d-flex align-items-center justify-content-end gap-2">
...
function calcModal(){
    ...
    if (otherAlloc > 0) {
        otherDiv.textContent = `(-${otherAlloc.toLocaleString()} Lazada)`;
    ...
    const barShopee = document.getElementById('mBarShopee');
    const barPos = document.getElementById('mBarPos');
    const labelShopee = document.getElementById('mRatioLabelShopee');
    const labelPos = document.getElementById('mRatioLabelPos');

    if (ratioText) {
        ratioText.textContent = `${shopeePct}% on Shopee`;
    }
    if (barShopee) {
        barShopee.style.width = `${shopeePct}%`;
        barShopee.setAttribute('aria-valuenow', shopeePct);
    }
    if (barPos) {
...
async function triggerManualSync() {
    ...
    try {
        ...
        for (let i = 0; i < totalItems; i += chunkSize) {
            ...
            try {
                const res = await fetch(`${window.BASE_URL}api/shopee/fetch_mapped_live_stocks.php`, {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
        ...
        try {
            await fetch(`${window.BASE_URL}api/shopee/log_stock_sync_summary.php`, {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
...
async function refreshLiveRow(id, btn) {
    ...
    try {
        ...
        if (data.success && data.updated && data.updated.length > 0) {
            ...
            renderMapped();
            updateSummary();
            EllaToast.success('Stock synced live from Shopee!');
        } else {
