...
try {
    ...
    // Get used mappings for active platform
    $usedStmt = $conn->prepare("SELECT pos_product_id, pos_unit_id, pos_bundle_set_id FROM lazada_product_mappings WHERE platform_name = ? AND (pos_product_id IS NOT NULL OR pos_bundle_set_id IS NOT NULL)");
    $usedStmt->execute([$platform]);
